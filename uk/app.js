...
function drawLabels() {
  ...
  if (flags.places) {
    ...
    for (const p of PLACES) {
      const pe = p.pe || p.pop || 0;
      // dupst: a real city's name is not redundant with its station label —
      // only suppress small dupst places, or anything once zoomed close
      // enough that the station label definitely renders (Aberdeen fix).
      if (p.dupst && flags.stn && (pe < 80000 || k >= 0.5)) continue;
      const minK = p.mk !== undefined ? p.mk
                 : (p.rank >= 4 ? 0 : p.rank >= 3 ? 0.10 : 0.45);
      if (k < minK) continue;
      if ((p.rank || 0) < 3 && !((p.pop || 0) >= 2000 || nearRail(p)))
        continue;
      const X = sx(p.x), Y = sy(p.y);
      if (X < -60 || X > SW + 60 || Y < -60 || Y > SH + 60) continue;
      // typography scales with population, not class band
      const fs = 9 + Math.min(7.5, 1.9 * Math.max(0, Math.log10(pe || 700) - 3));
      const font = (pe >= 180000 ? "700 " : "500 ") + fs.toFixed(1) + "px Georgia,serif";
      ctx.font = font;
...
/* ---------------- multi-map registry (../maps.json, optional) ----------
   Multi-region deployments put each map in its own dir (/uk/, /au/ ...)
   with a maps.json registry one level up. Single-map deployments — file://,
   panel builds, a missing/unreachable registry, or <2 entries — are left
   completely untouched: the fetch fails silently and the selector stays
   hidden, with the built-in UK title text as the fallback. */
let DOCTITLE = "Railway Map — Great Britain & Ireland";
function mapsInit() {
  fetch("../maps.json")
    .then((r) => { if (!r.ok) throw new Error("HTTP " + r.status); return r.json(); })
    .then((reg) => {
      const maps = ((reg && reg.maps) || []).filter((m) => m && m.id && m.name);
      if (maps.length < 2) return;
      // current region = the deepest path segment that names a map
      const segs = location.pathname.split("/")
        .filter((s) => s && s.toLowerCase() !== "index.html");
      const seg = segs[segs.length - 1];
      const cur = maps.find((m) => m.id === seg) ||
                  maps.find((m) => m.id === (reg.default || "")) || maps[0];
      if (cur.title) {
        $("titletext").textContent = cur.title;
        $("subtitle").textContent = cur.sub || "";
        DOCTITLE = cur.title + (cur.sub ? " — " + cur.sub : "");
      ...
      for (const m of maps) {
        ...
        if (m.id === cur.id) o.selected = true;
        sel.appendChild(o);
      }
      sel.addEventListener("change", () => {
        if (sel.value !== cur.id) location.href = "../" + sel.value + "/";
      });
      $("mapselwrap").hidden = false;
    })
    .catch(() => {});               // no registry -> single-map mode
}
mapsInit();
